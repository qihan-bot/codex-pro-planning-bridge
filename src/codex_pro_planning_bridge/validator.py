"""Validate a returned PLAN.md against local repository facts."""
...
def extract_dependency_references(markdown: str) -> list[DependencyReference]:
    """Extract dependencies and frameworks a plan explicitly proposes to use."""
    ...
    for reference in references:
        ...
        findings.append(
            Finding(
                ...
                reference=reference,
                status="OK" if found else "ERROR",
                detail="module is available locally" if found else "module is not available in the repository or declared dependencies",
            )
        )
    return findings
        ...
        if facts.has_dependency(reference.name):
            ...
            if reference.version and declared_version:
                ...
                if declared_match and declared_match.group(1).split(".")[0] != reference.version.split(".")[0]:
                    ...
                    continue
            findings.append(Finding("Dependency", reference.name, "OK", "dependency is declared in a supported manifest"))
            continue
...
def validate(
    ...
    *,
    ...
    """Validate a plan and write the local Markdown report."""
