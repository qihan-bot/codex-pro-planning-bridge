"""Build a small, explainable repository fact index for plan validation."""
...
try:
    from ._common import iter_project_files
except ImportError:  # Allow direct execution/import from scripts.
    from _common import iter_project_files  # type: ignore
...
class RepositoryFacts:
    """Facts discovered without executing repository code."""
    ...
    def has_symbol(self, reference: str) -> bool:
        ...
        if normalized in self.symbols:
            return True
        if "." in normalized:
            # A method may be discovered as a bare function by a language parser;
            # do not claim a qualified API exists unless it was indexed directly.
            return False
